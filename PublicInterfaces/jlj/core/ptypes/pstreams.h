...
#ifdef _MSC_VER
// 20090311, Guido Schmidt : 64bit problems
#ifndef _WIN64
#pragma pack(push)
#endif
#endif
...
// 20090311, Guido Schmidt : more compatible than int on 32/64 bit
const pintptr invhandle = -1;

// 20090311, Guido Schmidt : missing forward declaration
class ptpublic fdxoutstm;
...
// 20090311, Guido Schmidt : more compatible than int on 32/64 bit
    pintptr handle;         // used in many derivative classes
...
// 20090311, Guido Schmidt : more compatible than int on 32/64 bit
    pintptr  get_handle()                               { return handle; }
    int  get_bufsize()                              { return bufsize; }
    void set_bufsize(int newval);
// 20090311, Guido Schmidt : threading problems on SMP
    int  get_stmerrno()                             { int e; pexchange(&e, stmerrno); return e; }
    int  get_status()                               { int s; pexchange(&s, status); return s; }
...
protected:
    string filename;
// 20090311, Guido Schmidt : pintptr better than int
    pintptr syshandle;   // if not -1, assigned to handle in open() instead of opening a file by a name
    pintptr peerhandle;  // pipe peer handle, needed for closing the peer after fork() on unix
...
public:
    ...
    virtual string get_streamname();
// 20090311, Guido Schmidt : pintptr better than int
    pintptr get_syshandle()                     { return syshandle; }
    void set_syshandle(pintptr ihandle)         { close(); syshandle = ihandle; }
// 20090311, Guido Schmidt : pintptr better than int
    pintptr get_peerhandle()                    { return peerhandle; }
...
// 20090311, Guido Schmidt : very useful
    pintptr get_handle() const { return this->handle; }
};
...
// 20090311, Guido Schmidt : ptpublic was missing
class ptpublic unit_thread;
...
public:
 ...
 // 20090311, Guido Schmidt : duplicate symbol "main" on some unix
   virtual void unitmain();        // main code, called from run()

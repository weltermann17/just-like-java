...
// 20090311, Guido Schmidt : better socket performance per default
int defbufsize = 35*1460;
// 20090311, Guido Schmidt : threading problems with balance, just removed it
...
{
// 20090311, Guido Schmidt : threading problems 
	  pexchange(&status, newstat);
    if (onstatus != nil)
...
void iobase::open() 
{
    ...
    clear(deferrormsg);
    active = true;
// 20090311, Guido Schmidt : not needed
    bufalloc();
    doopen();
    chstat(IO_OPENED);
}
...
void iobase::close() 
{
// 20090311, Guido Schmidt : more re-entrant
    chstat(IO_CLOSED);	 
    if (!active)
        return;
// 20090311, Guido Schmidt : not needed
...
{
#ifdef _WIN32
    CloseHandle((HANDLE) pexchange((void**) &handle, (void*) invhandle));
#else
    ::close(pexchange((int*) &handle, invhandle));
#endif
}

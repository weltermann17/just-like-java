...
// 20090311, Guido Schmidt : improve performance for ipstream per default
const int DEFAULTBUFFERSIZE = 35*1460;
...
void ipstream::doopen()
{
    ...
    if (svsocket != invhandle)
    {
...
#if defined(__hpux)
        if ((handle = ::accept(svsocket, (sockaddr*)&sa, int*) &addrlen)) < 0)
#else
		...
		int sndbuf = DEFAULTBUFFERSIZE;
		int optlen = sizeof(sndbuf);
		::setsockopt(handle, SOL_SOCKET, SO_SNDBUF, (char*) &sndbuf, optlen);
		::setsockopt(handle, SOL_SOCKET, SO_RCVBUF, (char*) &sndbuf, optlen);

#if !defined(_WIN32)
		int tos;
#if defined(PLATEFORME_DS64)
		uint tossize = sizeof(tos);
		if (::getsockopt(handle, IPPROTO_IP, IP_TOS, &tos, &tossize) != 0)
#else
		ulong tossize = sizeof(tos);
		if (::getsockopt(handle, IPPROTO_IP, IP_TOS, &tos, &tossize) != 0)
#endif	
            error(uerrno(), "Couldn't get IP_TOS options");

		tos |= 0x08; // IPTOS_THROUGHPUT;
		if (::setsockopt(handle, IPPROTO_IP, IP_TOS, &tos, sizeof(tos)) != 0)
            error(uerrno(), "Couldn't set IP_TOS options");
#endif
    ...
    else
    {
        ...
        if ((handle = ::socket(sa.sin_family, SOCK_STREAM, 0)) < 0)
    		...
    		unsigned int sndbuf = DEFAULTBUFFERSIZE;
		    unsigned int optlen = sizeof(sndbuf);
		    ::setsockopt(handle, SOL_SOCKET, SO_SNDBUF, (char*) &sndbuf, optlen);
		    ::setsockopt(handle, SOL_SOCKET, SO_RCVBUF, (char*) &sndbuf, optlen);
...
{
	  pintptr sockethandle = (pintptr) pexchange((void**) &handle, (void*) invhandle);
...
{
	  if (eof) error(uerrno(), "Couldn't read");
    int ret;
...
bool ipstream::waitfor(int timeout)
{
    ...
    if (bufsize > 0 && bufend > bufpos)
        return true;
    return psockwait((int) handle, timeout);
}

...
#if defined(_AIX) || defined (__HP_aCC)
	long altzone = -3600; // will only work in CET
#endif

#if defined(__sun__) || defined(__IBMCPP__) || defined (__xlC__) || defined (__HP_aCC)
#ifdef PTYPES_ST
...
#endif
    {
#if defined(__IBMCPP__) || defined (__xlC__) || defined (__HP_aCC)
#ifdef DEBUG
		static bool once = true;
		if (once)
		{
			once = false;
			fprintf(stdout, "file : %s, line : %d in tzoffset() using 'altzone' on AIX/HPUX does not work correctly (timezone = %d)\n", __FILE__, __LINE__, timezone);
                        fflush(stdout);
		}
#endif
#endif

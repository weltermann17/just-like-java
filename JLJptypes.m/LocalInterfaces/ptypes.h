...
#ifdef _MSC_VER
// 20090311, GSM : unnecessary performance penalty, 64bit problems
#pragma pack(push)
...
#if ((__GNUC__ == 3) && (__GNUC_MINOR__ >= 3)) || (__GNUC__ == 4) || defined(__hpux) || defined (_AIX)
...
// 20090311, GSM : needed when called in const context
    const X&   doget(int index) const       { return ((X*)list)[index]; }
...
// 20090311, GSM : won't compile on AIX
#ifdef _AIX
    { return (T*)addref(c); }
#else
